...
if($_POST['action'] == 'insert_chat') {
	$chat->insertChat($_POST['to_user_id'], $_SESSION["usuario"]['idUsuario'], $_POST['chat_message']);
}
if($_POST['action'] == 'show_chat') {
	$chat->showUserChat($_SESSION["usuario"]['idUsuario'], $_POST['to_user_id']);
}
if($_POST['action'] == 'update_user_chat') {
	$conversation = $chat->getUserChat($_SESSION["usuario"]['idUsuario'], $_POST['to_user_id']);
	$data = array(
...
if($_POST['action'] == 'update_unread_message') {
	$count = $chat->getUnreadMessageCount($_POST['to_user_id'], $_SESSION["usuario"]['idUsuario']);
	$data = array(
		"count" => $count			
	);
	echo json_encode($data);
