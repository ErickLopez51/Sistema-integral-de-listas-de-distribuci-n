...
class Chat{
	...
	public function getUserChat($from_user_id, $to_user_id) {
		...
		foreach($userChat as $chat){
			...
			if($chat["sender_userid"] == $from_user_id) {
				...
				$conversation .= '<img width="22px" height="22px" src="/sild/Public/userpics/'.$fromUserAvatar.'" alt="" />';
			} else {
				$conversation .= '<li class="replies">';
				$conversation .= '<img width="22px" height="22px" src="/sild/Public/userpics/'.$toUserAvatar.'" alt="" />';
			}			
	...
	public function showUserChat($from_user_id, $to_user_id) {		
		...
		foreach ($userDetails as $user) {
			...
			$userSection = '<img src="/sild/Public/userpics/'.$user['avatar'].'" alt="" />
				<p>'.$user['username'].'</p>';
		}		
